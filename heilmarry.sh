...
echo -e "${red}\nSQLi Test.${reset}"
read -p "${magenta}Do you want to check for SQLi: [y/n] ${reset}" input

if [[ $input == 'y' ]]; then
	echo -e "\n${red}You should not blindly trust the results. There may be a lot of false positives.${reset}\n"
	echo -e "\n${yellow}Ignore the output below, if there are any suspects, the final list of suspected URLs will be at output/$domain/sqli-suspects.txt${reset}\n"
	count=0
	while IFS= read -r url; do
		 echo $url | qsreplace "'" > output/$domain/temp.txt
		 rm output/$domain/temp.txt
		 if [[ $cookie != "" ]]; then
			 if [[ $(curl --silent --cookie $cookie $url | grep "sql" ) != '' ]]; then
	...
	if [[ -f output/$domain/sqli-suspects.txt ]]; then
		...
		echo "${green}The suspected URLs are stored in output/$domain/sqli-suspects.txt${reset}"
	else
		echo "${yellow}No injections found! :( ${reset}"
	fi
